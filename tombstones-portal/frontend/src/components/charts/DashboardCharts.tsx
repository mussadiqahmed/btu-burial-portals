...
export function ExpectedVsActualChart({ data }: { data: TrendRow[] }) {
  const chartData = data.map((row) => ({
    label: shortMonthLabel(Number(row.month), Number(row.year)),
    actual: Number(row.paid_amount ?? row.total_paid) || 0,
    expected: Number(row.expected_amount ?? row.total_expected) || 0,
  }));
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <LineChart data={chartData}>
        ...
        />
        <Line type="monotone" dataKey="expected" stroke="#8B5CF6" strokeWidth={2} dot={false} />
