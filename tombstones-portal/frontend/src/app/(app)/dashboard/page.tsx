        ...
        <>
          ...
          >
            ...
            <KpiCard
              ...
              value={(stats.active_members ?? 0).toLocaleString()}
              icon={UserCheck}
              accent="success"
            />
            <KpiCard
              label="Inactive Members"
              value={(stats.inactive_members ?? 0).toLocaleString()}
              icon={UserXIcon}
              accent="violet"
            />
            <KpiCard
              label="Expected Collections"
              value={formatMoney(stats.expected_collections_total)}
              icon={Wallet}
              accent="violet"
            />
            <KpiCard
            ...
            />
            <KpiCard
              label="Collection Difference"
              value={formatMoney(stats.collections_difference)}
              icon={TrendingUp}
              accent="primary"
            />
            <KpiCard
              label="Underpaid Members"
              value={(stats.underpaid_count ?? 0).toLocaleString()}
              icon={TriangleAlert}
              accent="danger"
            />
            <KpiCard
            ...
            <ChartCard title="Active vs Inactive" subtitle="Member status distribution">
              <div className="flex items-center justify-center">
                <ActiveInactivePieChart
                  ...
                  inactive={stats.inactive_members ?? 0}
                />
              </div>
          ...
          <div className="mb-8 grid gap-4 sm:grid-cols-2">
            <div className="glass-card border-success/20 bg-success-muted/30 p-5">
              ...
              <p className="mt-1 text-xs text-muted">active members with a payment record</p>
            </div>
            <div className="glass-card border-warning/20 bg-warning-muted/30 p-5">
              ...
              <p className="mt-1 text-xs text-muted">active members missing payment</p>
            </div>
          </div>
