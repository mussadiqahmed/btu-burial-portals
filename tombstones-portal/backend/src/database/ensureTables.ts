...
export async function ensureTables() {
  ...
  const collectionColumns: Array<[string, string]> = [
    ['expected_amount', 'DECIMAL(12, 2) NULL'],
    ['amount_difference', 'DECIMAL(12, 2) NULL'],
    ['comparison_status', "ENUM('match', 'underpaid', 'overpaid') NULL"],
  ];

  for (const [column, definition] of collectionColumns) {
    const [exists] = await pool.execute<import('mysql2/promise').RowDataPacket[]>(
      `SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'tombstone_monthly_collections' AND COLUMN_NAME = ?`,
      [column]
    );
    if (Number(exists[0]?.cnt) === 0) {
      await pool.execute(
        `ALTER TABLE tombstone_monthly_collections ADD COLUMN ${column} ${definition}`
      );
    }
  }
