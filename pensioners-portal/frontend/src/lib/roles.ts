...
export const navigation: NavItem[] = [
  ...
  { name: 'Pensioners', href: '/pensioners', icon: Users },
  { name: 'Uploads', href: '/upload', icon: UploadCloud, writeOnly: true },
  { name: 'Reports', href: '/reports', icon: FileText },
  { name: 'Analytics', href: '/analytics', icon: BarChart3 },
  { name: 'Missing Collections', href: '/reports/missing', icon: TriangleAlert },
...
export function getNavigationForRole(role?: string | null): NavItem[] {
  ...
  return navigation.filter((item) => {
    ...
    if (item.writeOnly && !canWritePensioners(r)) return false;
    return true;
  });
}
...
export function canAccessPath(role: string | undefined | null, pathname: string): boolean {
  ...
  if (path.startsWith('/upload') && !canWritePensioners(r)) return false;
